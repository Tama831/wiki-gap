...
> 1 つの Wikimedia アカウントに紐づく形で運用される設計です。別の人が自分用に使うときは、下の「[セットアップ](#セットアップ)」へ（いちばんラクな方法から並べています）。コマンドに慣れている人向けの手短版は「[自分用にセットアップする](#自分用にセットアップする)」にあります。
> リポジトリ内に作者個人 (Wikipedia ユーザ名・GitHub 等) を示唆する記述があれば、すべて環境変数または初回セットアップで上書きできるようにしてあります。

## セットアップ

パソコンのコマンドに自信がなくても大丈夫。ラクな方法から順に並べています。

### 方法1（おすすめ）：AI コーディングツールに任せる

コマンドに不安があるなら、AI のコマンドラインツールに任せてしまうのが最短です。

1. AI CLI のどれかをインストール（[Claude Code](https://claude.com/claude-code) / Codex CLI / Gemini CLI など）
2. 開いて、こう頼むだけ:
   > GitHub にある `Tama831/wiki-gap` を自分のパソコンで動かしたいです。最初から手伝ってください。あわせて Wikipedia アカウントの作り方も教えてください。
3. AI が手順を1つずつ案内し、コマンドも実行してくれます。エラーが出たら、その文をそのまま貼れば直し方を教えてくれます。

### 方法2：自分の手でやる（ターミナルから）

<details>
<summary><b>クリックで全手順を開く（ターミナルの開き方から、初めての方向け）</b></summary>
...
- どうしても進めない → 出てきたエラー文をそのままコピーして、上の「方法1」の AI に貼ると直し方を教えてくれます
...
> コマンド操作に慣れている人向けの手短版です。はじめての方は上の「セットアップ」をどうぞ。
