...
chunks_json に書き込む 1 回限りのスクリプト。

ポリシー:
  - 直訳でなく日本の医学文脈で自然な表現
  - 出典 (<ref>...</ref>) はそのまま残す
  - リンク [[foo]] は ja Wikipedia に対応する記事があれば置換
  - 表 (wikitable) は構造そのまま、ヘッダだけ翻訳
  - カテゴリは ja Wikipedia 規約に合わせる
"""
...
# chunk_id -> ja wikitext translation
TRANSLATIONS: dict[int, str] = {}

# ─────────────────────────────────────────────────────────────────
# id=0 (intro)
# ─────────────────────────────────────────────────────────────────
TRANSLATIONS[0] = r"""{{Short description|単一の患者を対象とする臨床試験}}
...
<small>「準実験」は因果関係を確定的には示せないことを意味し、「実験」は確定的に示しうることを意味する。</small>

[[File:Single subject blood pressure example.png|center|thumb|500px|A-A<sup>1</sup>-A 型 N-of-1 試験の合成データ例。1〜30 日目、61〜90 日目、121〜150 日目に被験者は[[高血圧]]治療薬を服用し、それ以外の期間はプラセボを服用している。正常な[[収縮期血圧]]は 120 mmHg をやや下回る程度である。]]
...
# id=3 == Examples ==
# ─────────────────────────────────────────────────────────────────
TRANSLATIONS[3] = r"""== 適用例 ==
N-of-1試験は通常、[[慢性疾患]]を対象とする治療への個別の反応性を評価するために用いられる。<ref>{{Cite journal |last1=Duan |first1=Naihua |last2=Kravitz |first2=Richard L. |last3=Schmid |first3=Christopher H. |date=2013-08-01 |title=Single-patient (n-of-1) trials: a pragmatic clinical decision methodology for patient-centered comparative effectiveness research |journal=Journal of Clinical Epidemiology |series=Methods for Comparative Effectiveness Research/Patient-Centered Outcomes Research: From Efficacy to Effectiveness |volume=66 |issue=8, Supplement |pages=S21–S28 |doi=10.1016/j.jclinepi.2013.04.006 |pmid=23849149 |pmc=3972259 |issn=0895-4356}}</ref> このデザインは、[[変形性関節症]]、慢性[[末梢神経障害|神経障害性]]疼痛、[[注意欠如・多動症]]など多様な疾患を持つ患者の最適な治療を決定するために有効に活用されてきた。<ref>{{cite journal | vauthors = Scuffham PA, Nikles J, Mitchell GK, Yelland MJ, Vine N, Poulos CJ, Pillans PI, Bashford G, del Mar C, Schluter PJ, Glasziou P | display-authors = 6 | title = Using N-of-1 trials to improve patient management and save costs | journal = Journal of General Internal Medicine | volume = 25 | issue = 9 | pages = 906–13 | date = September 2010 | pmid = 20386995 | pmc = 2917656 | doi = 10.1007/s11606-010-1352-7 | url = http://iospress.metapress.com/content/t51wg3207328hv38/?genre=article&issn=1387-2877&volume=21&issue=3&spage=967 | archive-url = https://archive.today/20130923221601/http://iospress.metapress.com/content/t51wg3207328hv38/?genre=article&issn=1387-2877&volume=21&issue=3&spage=967 | archive-date = 2013-09-23 }}</ref>

N-of-1 デザインは観察的にも実施可能であり、健康関連行動や症状の個人内変動を縦断的に記述することもできる。N-of-1 観察研究のデータには複雑な統計解析が必要となるが、初学者向けに 10 ステップの簡易な手順も紹介されている。<ref>{{cite journal |last1=McDonald |first1=S |last2=Vieira |first2=R |last3=Johnston |first3=D W. |title=Analysing N-of-1 observational data in health psychology and behavioural medicine: a 10-step SPSS tutorial for beginners |journal=Health Psychology and Behavioral Medicine |date=1 January 2020 |volume=8 |issue=1 |pages=32–54 |doi=10.1080/21642850.2019.1711096|pmid=34040861 | pmc=8114402 |doi-access=free }}</ref> また、N-of-1 観察研究を後続の N-of-1 試験設計に活かすため、[[因果推論]]の[[反実仮想]]的手法を応用する研究も進んでいる。<ref>{{cite journal | vauthors = Daza EJ | title = Causal Analysis of Self-tracked Time Series Data Using a Counterfactual Framework for N-of-1 Trials | journal = Methods of Information in Medicine | volume = 57 | issue = 1 | pages = e10–e21 | date = February 2018 | pmid = 29621835 | pmc = 6087468 | doi = 10.3414/ME16-02-0044 | doi-access = free }}</ref><ref>{{cite journal | vauthors = Daza EJ, Matias I, Schneider L | title = Model-Twin Randomization (MoTR) for Estimating the Recurring Individual Treatment Effect | journal = Statistics in Medicine | volume = 44 | issue = 25-27 | article-number = e70290 | date = November 2025 | pmid = 41222445 | doi = 10.1002/sim.70290 }}</ref>

N-of-1試験は増加傾向にあるが、近年のシステマティックレビューによれば、これらの研究における統計解析は、研究のあらゆる段階でより方法論的・統計的厳密性を高める余地がある。<ref>{{cite journal | vauthors = Shaffer JA, Kronish IM, Falzon L, Cheung YK, Davidson KW | title = N-of-1 Randomized Intervention Trials in Health Psychology: A Systematic Review and Methodology Critique | journal = Annals of Behavioral Medicine | volume = 52 | issue = 9 | pages = 731–742 | date = August 2018 | pmid = 30124759 | pmc = 6128372 | doi = 10.1093/abm/kax026 }}</ref>

"""

# ─────────────────────────────────────────────────────────────────
# id=4 == The Quantified Self ==
# ─────────────────────────────────────────────────────────────────
TRANSLATIONS[4] = r"""== クオンティファイド・セルフ ==
[[クオンティファイド・セルフ|Quantified Self]] という文化的潮流の高まりに伴い、N=1 試験に類する個人実験が広がりを見せており、それらに関する詳細な報告も増えている。この傾向の背景には、データの収集と解析がますます容易になったこと、そして個人がそうしたデータを手軽に発信できるようになったことがある。<ref>{{cite journal | vauthors = Swan M | title = The Quantified Self: Fundamental Disruption in Big Data Science and Biological Discovery | journal = Big Data | volume = 1 | issue = 2 | pages = 85–99 | date = June 2013 | pmid = 27442063 | doi = 10.1089/big.2012.0002 | doi-access = free }}</ref>

著名な提唱者かつ実践者として[[セス・ロバーツ]](Seth Roberts)が知られている。彼は自身のブログで自己実験の知見を公表し、後にこの自己実験から導いた結論をもとに『''[[The Shangri-La Diet]]''』を出版した。

"""

# ─────────────────────────────────────────────────────────────────
# id=5 == Global networks ==
# ─────────────────────────────────────────────────────────────────
TRANSLATIONS[5] = r"""== 国際的なネットワーク ==
「N-of-1試験および単一症例デザインのための国際協同ネットワーク」(International Collaborative Network for N-of-1 Trials and Single-Case Designs, ICN)<ref>{{Cite web |title=International Collaborative Network for N-of-1 Trials and Single-Case Designs |url=https://www.nof1sced.org/ |access-date=2024-07-09 |website=N-of-1 and SCED |language=en}}</ref> は、これらの方法論に関心を持つ臨床家・研究者・一般生活者のための国際ネットワークである。ICN には世界 30 か国以上から 400 名を超えるメンバーが参加している。ICN は 2017 年に設立され、現在は Jane Nikles と Suzanne McDonald が共同議長を務めている。
...
# id=6 == See also ==
# ─────────────────────────────────────────────────────────────────
TRANSLATIONS[6] = r"""== 関連項目 ==
* {{annotated link|応用行動分析}}
...
# ─────────────────────────────────────────────────────────────────
# id=8 == Further reading ==
# ─────────────────────────────────────────────────────────────────
TRANSLATIONS[8] = r"""== 参考文献 ==
{{refbegin}}
...
def main() -> int:
    ...
    for c in chunks:
        ...
        if cid in TRANSLATIONS:
            new_dst = TRANSLATIONS[cid].rstrip() + "\n"
            old_dst = c.get("dst", "") or ""
            if old_dst.strip():
                # 既に手で書かれているなら上書きしない
                print(f"  skip id={cid} (already has dst, {len(old_dst)} chars)")
                n_skipped += 1
                continue
            c["dst"] = new_dst
            print(f"  set  id={cid} heading={c['heading']!r:40} ({len(new_dst)} chars)")
            n_updated += 1
        else:
            print(f"  skip id={cid} (no translation provided)")
